...
# ======================
#   ПАРСЕР ТЕСТОВ (исправленный)
# ======================

def parse_quiz(text):
    questions = []

    # Делим по номерам: 1. 2. 3.
    blocks = re.split(r'(?=\n?\d+\.\s)', text.strip())
    ...
    for block in blocks:
        ...
        for line in lines[1:]:
            ...
            if re.match(r'^(ответ|правильный ответ|answer)[:\-]?', low):
                correct_raw = stripped.split(':', 1)[-1].strip()

            # Варианты вида a) b)
            elif re.match(r'^[aаbбвcгdеe]\)', low):
                options.append(re.sub(r'^[aаbбвcгdеe]\)\s*', '', stripped, flags=re.I))

            # Если вдруг Telegram склеил следующий номер вопроса
        ...
        # Определяем номер правильного ответа
        correct_index = []
        for ans in re.split(r'[,\s]+', correct_raw):
            ans = ans.lower().strip()
            index = {'а': 1, 'б': 2, 'в': 3, 'г': 4, 'д': 5,
                     'a': 1, 'b': 2, 'c': 3, 'd': 4, 'e': 5}
            if ans in index:
